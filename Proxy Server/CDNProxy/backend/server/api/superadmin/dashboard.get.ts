// @ts-nocheck
import { logger } from '~/utils/logger'
import { defineEventHandler, createError, getHeader } from 'h3'
import { requireAdminAuth } from '../../../utils/hybrid-auth'

export default defineEventHandler(async (event) => {
  try {
    // Verificar se há token de autenticação antes de processar
    const authHeader = getHeader(event, 'authorization')
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      // Retornar erro 401 em vez de deixar o Nuxt.js servir a página
      throw createError({
        statusCode: 401,
        statusMessage: 'Token de autenticação necessário',
        data: {
          success: false,
          error: 'Token de autenticação necessário'
        }
      })
    }
    
    // Authenticate superadmin
    const { user, userProfile } = await requireAdminAuth(event, 'SUPERADMIN')
    
    // Retornar dados do dashboard do superadmin
    return {
      success: true,
      data: {
        user: {
          id: userProfile.id,
          email: userProfile.email,
          name: userProfile.name,
          role: userProfile.role
        },
        stats: {
          // Adicionar estatísticas relevantes para o superadmin
          totalUsers: 0,
          totalDomains: 0,
          totalTransactions: 0,
          usersByRole: {
            superadmin: 0,
            admin: 0,
            user: 0
          }
        }
      }
    }
  } catch (error: any) {
    // Garantir que erros sejam retornados como JSON
    event.node.res.setHeader('Content-Type', 'application/json')
    
    if (error.statusCode) {
      throw error
    }
    
    throw createError({
      statusCode: 500,
      statusMessage: 'Erro interno do servidor',
      data: {
        success: false,
        error: 'Erro interno do servidor'
      }
    })
  }
})