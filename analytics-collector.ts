const logger = consola.withTag('analytics-collector')
...
class AnalyticsQueue {
  ...
  private async processBatch(batch: Array<{ type: string; data: any }>) {
    if (!supabase) {
      logger.warn('Supabase not configured, skipping analytics batch processing')
      return
    }
    ...
    try {
      ...
      // For example:
      logger.info('Processed batch', { accessLogs: accessLogs.length, hlsMetrics: hlsMetrics.length, streamingMetrics: streamingMetrics.length })
      // Similarly for other console calls.
    } catch (error) {
...
// Função para adicionar IP ao cache de geolocalização
async function addToIPCache(ip: string, geoData: any) {
  try {
    if (!supabase) return
    ...
    if (!existing) {
      // Adicionar ao cache com expiração de 30 dias
      ...
      console.info(`IP ${ip} adicionado ao cache de geolocalização`)
    }
  } catch (error) {
    console.error('Erro ao adicionar IP ao cache:', error)
  }
}
...
export async function collectAccessLog(event: H3Event, options: {
  ...
  try {
    ...
    const accessLogData: AccessLogData = {
      ...
      bytes_sent: options.bytesSent || options.bytesTransferred
    }
    
    analyticsQueue.add('access_log', accessLogData)
    
    // Se o IP não estiver no cache de geolocalização, adicionar
    if (geo.country && geo.city) {
      await addToIPCache(ipDetection.ip, geo)
    }
  } catch (error) {
    console.error('Error collecting access log:', error)
  }
}
...
export async function collectHLSMetrics(event: H3Event, options: {
  ...
  try {
    ...
    console.error('Error collecting HLS metrics:', error)
  }
}
...
export function withAnalytics(domainId: string, domain: string) {
  return async (event: H3Event, next: () => Promise<any>) => {
    ...
    try {
      ...
      // Coletar métricas específicas para streaming se necessário
      const deviceType = detectDeviceType(getHeader(event, 'user-agent') || '')
      if (deviceType === 'iptv' || path.includes('.m3u8') || path.includes('.ts')) {
